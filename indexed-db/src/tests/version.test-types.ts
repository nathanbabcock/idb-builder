...
void function testSuccessiveVersionsMustBeGreater() {
  ...
  createMigrations()
    ...
    .version(3, v =>
      v.createObjectStore({ name: 'comments', schema: z.object({ id: z.string() }) })
    )
  ...
  createMigrations()
    ...
    .version(1, v => v.createObjectStore({ name: 'tags', schema: z.object({ id: z.string() }) }))
}
...
void function testInvalidVersionSameAsPrevious() {
  createMigrations()
    ...
    .version(1, v => v.createObjectStore({ name: 'posts', schema: z.object({ id: z.string() }) }))
}

void function testInvalidVersionLessThanPrevious() {
  createMigrations()
    .version(5, v => v.createObjectStore({ name: 'users', schema: z.object({ id: z.string() }) }))
    // @ts-expect-error Version 3 must be greater than previous version 5
    .version(3, v => v.createObjectStore({ name: 'posts', schema: z.object({ id: z.string() }) }))
}
...
void function testInvalidVersionZeroWhenPreviousPositive() {
  createMigrations()
    ...
    .version(0, v => v.createObjectStore({ name: 'posts', schema: z.object({ id: z.string() }) }))
}
