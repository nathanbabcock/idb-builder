...
<<< @/samples/composite-keys.sample.ts{ts twoslash}
...
<<< @/samples/composite-keys-type-mismatch.sample.ts{ts twoslash}

### Key array length

You must specify a value for every array member in a composite key.

<<< @/samples/composite-keys-array-length.sample.ts{ts twoslash}